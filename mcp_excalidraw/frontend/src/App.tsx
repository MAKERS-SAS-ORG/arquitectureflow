...
function App(): JSX.Element {
  ...
  const [initialLibraryItems, setInitialLibraryItems] = useState<LibraryItems>(() => {
    const savedLibraries = localStorage.getItem('excalidraw-libraries');
    if (savedLibraries) {
      try {
  ...
  const loadBackendLibraries = async (): Promise<void> => {
    try {
      ...
      if (data.success && data.libraries && data.libraries.length > 0) {
        const fetchPromises = data.libraries.map(async (url: string) => {
          try {
            const res = await fetch(url);
            const libData = await res.json();
            if (libData && libData.type === 'excalidrawlib' && Array.isArray(libData.libraryItems)) {
              return libData.libraryItems;
            }
          } catch (e) {
            console.error('Error fetching library at', url, e);
          }
          return [];
        });
      ...
      <div className="canvas-container">
        <Excalidraw
          ...
          onLibraryChange={(libraryItems) => {
            // Protect against Excalidraw firing an empty array on the very first mount cycle 
            // before the initialData or updateLibrary has had a chance to populate the internal state.
            if (libraryItems.length === 0 && initialLibraryItems.length > 0) {
              const saved = localStorage.getItem('excalidraw-libraries');
              if (saved && saved !== '[]') {
                return; // Skip overwriting if we haven't loaded yet
              }
            }
            localStorage.setItem('excalidraw-libraries', JSON.stringify(libraryItems))
          }}
        />
      </div>
    </div>
