...
// Serve static extension libraries
const extDir = path.join(__dirname, '../ext');
app.use('/ext', express.static(extDir));
...
// Get available extension libraries
app.get('/api/libraries', async (req: Request, res: Response) => {
  try {
    ...
    const libs = files.filter(f => f.endsWith('.excalidrawlib'));
    res.json({
      success: true,
      libraries: libs.map(f => `/ext/${f}`)
    });
  } catch (error) {
